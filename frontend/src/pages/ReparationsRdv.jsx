...
const EtiquettePreviewPageContentRdv = React.forwardRef(({ repairData }, ref) => {
  ...
  useEffect(() => {
    ...
    if (companyInfoRaw) {
      try {
        ...
        console.error("Failed to parse companyInfo from localStorage for RDV etiquette", e);
      }
    }
  }, [repairData]);
  ...
  return (
    <div ref={ref} className="p-2 m-0 text-xs bg-white" style={{ width: '150px', border: '1px solid black' }}>
      ...
      <p>RDV: {repairData.dateRendezVous ? new Date(repairData.dateRendezVous).toLocaleDateString('fr-FR') : 'N/A'}</p>
...
EtiquettePreviewPageContentRdv.displayName = "EtiquettePreviewPageContentRdv";


const ReparationsRdv = ({ onSave, generateNumeroReparation, availableStocks = [] }) => {
  const recuPreviewRef = useRef();
  const typeString = 'Réparation sur RdV';
  ...
  const initialFormData = {
    ...
    paye: 0,
    resteAPayer: 0,
    dateRendezVous: formatDateForInput(new Date(new Date().setDate(new Date().getDate() + 1))),
    dateRetrait: '',
  ...
  const handlePrintTicket = useReactToPrint({
    ...
    documentTitle: `Recu_${formData.numeroReparation || 'reparation_rdv'}`,
    onAfterPrint: () => toast({ title: "Impression Ticket RDV", description: "Ticket RDV envoyé à l'imprimante." }),
  });

  const handlePrintEtiquette = useReactToPrint({
    content: () => etiquettePreviewRef.current,
    documentTitle: `Etiquette_${formData.numeroReparation || 'reparation_rdv'}`,
    onAfterPrint: () => toast({ title: "Impression Étiquette RDV", description: "Étiquette RDV envoyée à l'imprimante." }),
  });
  ...
  useEffect(() => {
    ...
    const newTotal = pannesTotal + piecesTotal;
    const newResteAPayer = newTotal - (parseFloat(formData.paye) || 0);
    setFormData(prev => ({
      ...prev,
      total: newTotal,
      resteAPayer: newResteAPayer,
      etatPaiement: newResteAPayer <= 0 ? 'Soldé' : 'Non soldé'
    }));
  }, [formData.montants, formData.paye, formData.piecesRechange, availableStocks]);
  ...
  const handleSubmit = (e) => {
    ...
    const repairToSend = {
      numeroReparation: formData.numeroReparation, // Ajouté pour correspondre au schéma backend
      type_reparation: 'rdv',
      ...
      date_rendez_vous: formData.dateRendezVous ? new Date(formData.dateRendezVous) : undefined,
      date_retrait: formData.dateRetrait ? new Date(formData.dateRetrait) : undefined,
  ...
  // Correction : forçage du type number pour paye avant d'envoyer à RecuPreview
  const recuPreviewData = {
    ...formData,
    paye: typeof formData.paye === 'string' ? parseFloat(formData.paye) || 0 : formData.paye,
    total: typeof formData.total === 'string' ? parseFloat(formData.total) || 0 : formData.total,
    montants: Array.isArray(formData.montants) ? formData.montants.map(m => typeof m === 'string' ? parseFloat(m) || 0 : m) : [],
  };
    ...
    >
      ...
      <div className="flex flex-col md:flex-row flex-1 overflow-hidden">
        <form onSubmit={handleSubmit} className="w-full md:w-3/5 p-4 space-y-3 overflow-y-auto md:border-r border-gray-200">
          <div>
            <Label htmlFor="numeroReparation" className="text-xs font-medium text-gray-600">N° Réparation RDV</Label>
            <Input
              id="numeroReparation"
          ...
          <div>
            <Label htmlFor="appareil" className="text-xs font-medium text-gray-600">Appareil</Label>
            <select
              id="appareil"
              name="appareil"
            ...
            {formData.piecesRechange.map((piece, index) => (
              <div key={index} className="flex items-center space-x-2 mb-1.5">
                ...
                <Input
                  ...
                  className="w-1/4 text-sm py-1.5"
                />
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <Input id="total" name="total" type="number" value={formData.total} readOnly className="w-full text-sm py-1.5 bg-gray-100 font-semibold" />
            </div>
            <div>
              <Label htmlFor="paye" className="text-xs font-medium text-gray-600">Payé</Label>
              <Input id="paye" name="paye" type="number" inputMode="decimal" pattern="[0-9]*" value={formData.paye === 0 ? '' : formData.paye} onChange={handleInputChange} className="w-full text-sm py-1.5 no-spinner" required style={{ MozAppearance: 'textfield' }} onWheel={e => e.target.blur()} />
            </div>
            <div>
              <Label htmlFor="resteAPayer" className="text-xs font-medium text-gray-600">Reste à Payer</Label>
              <Input id="resteAPayer" name="resteAPayer" type="number" value={formData.resteAPayer} readOnly className="w-full text-sm py-1.5 bg-gray-100 font-semibold" />
            </div>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <Label htmlFor="dateRendezVous" className="text-xs font-medium text-gray-600">Date de RDV</Label>
              <Input id="dateRendezVous" name="dateRendezVous" type="date" value={formData.dateRendezVous} onChange={handleInputChange} className="w-full text-sm py-1.5" required/>
            </div>
          </div>
          <div>
            <Label htmlFor="etatPaiement" className="text-xs font-medium text-gray-600">État Paiement</Label>
            <Input id="etatPaiement" name="etatPaiement" type="text" value={formData.etatPaiement} readOnly className="w-full text-sm py-1.5 bg-gray-100" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <Label htmlFor="statut" className="text-xs font-medium text-gray-600">Statut Réparation</Label>
              <Select value={formData.statut} onValueChange={(value) => setFormData({ ...formData, statut: value })}>
                <SelectTrigger className="w-full text-sm py-1.5"><SelectValue placeholder="Sélectionner statut" /></SelectTrigger>
                <SelectContent>
          ...
          <div className="flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3 pt-3 sticky bottom-0 bg-gray-50 py-3 border-t border-gray-200 mt-auto">
            <Button type="button" variant="outline" onClick={resetForm} className="w-full sm:w-auto text-xs bg-red-100 text-red-700 hover:bg-red-200 border-red-200">Effacer</Button>
            <Button type="submit" className="w-full sm:w-auto text-xs bg-green-600 hover:bg-green-700 text-white">Enregistrer</Button>
          </div>
        </form>
        ...
        <div className="w-full md:w-2/5 p-1 md:p-2 bg-white overflow-y-auto flex flex-col items-center justify-start">
          ...
          <div style={{ display: "none" }}>
            <EtiquettePreviewPageContentRdv ref={etiquettePreviewRef} repairData={formData} />
          </div>
        </div>
      </div>
    </motion.div>
...
// Ajout PropTypes pour EtiquettePreviewPageContentRdv
EtiquettePreviewPageContentRdv.propTypes = {
