<?php $__env->startSection('page-title', 'Service Après-Vente (SAV)'); ?>
...
<div class="space-y-6" x-data="savPage()">
        ...
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    <?php $__currentLoopData = $savs; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sav): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 text-sm font-medium text-blue-600"><?php echo e($sav->numeroSAV); ?></td>
                        <td class="px-4 py-3 text-sm">
                            <div><?php echo e($sav->client_nom); ?></div>
                            <div class="text-xs text-gray-500"><?php echo e($sav->client_telephone); ?></div>
                        </td>
                        <td class="px-4 py-3 text-sm"><?php echo e($sav->appareil_marque_modele); ?></td>
                        <td class="px-4 py-3 text-sm"><?php echo e($sav->numeroReparationOrigine ?? '-'); ?></td>
                        <td class="px-4 py-3">
                            <?php if($sav->sous_garantie): ?>
                        ...
                        <td class="px-4 py-3">
                            <span class="px-2 py-0.5 text-xs font-semibold rounded-full
                                <?php echo e($sav->statut === 'Résolu' ? 'bg-green-100 text-green-800' :
                                   ($sav->statut === 'Refusé' ? 'bg-red-100 text-red-800' :
                                   ($sav->statut === 'En cours' ? 'bg-yellow-100 text-yellow-800' :
                                   'bg-gray-100 text-gray-800'))); ?>">
                                <?php echo e($sav->statut); ?>

                            </span>
                        </td>
                        <td class="px-4 py-3 text-sm"><?php echo e($sav->date_creation ? $sav->date_creation->format('d/m/Y') : 'N/A'); ?></td>
                        <td class="px-4 py-3 text-center space-x-1">
                            <button @click="openEdit(<?php echo e($sav->toJson()); ?>)"
                                    class="text-blue-600 hover:text-blue-800 p-1">
                                <i class="fas fa-pen"></i>
                            </button>
                            <form action="<?php echo e(route('sav.destroy', $sav->id)); ?>" method="POST" class="inline"
    ...
    <div x-show="showCreate" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4" @click.self="showCreate = false">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <form action="<?php echo e(route('sav.store')); ?>" method="POST" class="p-6 space-y-4">
                <?php echo csrf_field(); ?>
                <div>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /data/projects/node.js/momotechservice/momotech-app/resources/views/dashboard/sav.blade.php ENDPATH**/ ?>