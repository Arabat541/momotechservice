...
@php $reportTitle = 'FACTURES FOURNISSEURS'; @endphp
...
<table>
    <thead>
        <tr>
            ...
            <th>Fournisseur</th>
            <th>Date</th>
            <th class="right">Montant TTC (F)</th>
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ optional($inv->supplier)->nom ?? '—' }}</td>
        ...
        @empty
        <tr><td colspan="7" style="text-align:center; color:#94a3b8; padding:12px;">Aucune facture fournisseur.</td></tr>
        @endforelse
    </tbody>
    <tfoot>
        <tr>
            <td colspan="3">TOTAL ({{ $rows->count() }} facture(s))</td>
            <td class="right">{{ number_format($rows->sum('montant_ttc'), 0, ',', ' ') }}</td>
            <td class="right">{{ number_format($rows->sum('montant_paye'), 0, ',', ' ') }}</td>
            <td class="right">{{ number_format($rows->sum('reste_a_payer'), 0, ',', ' ') }}</td>
            <td></td>
        </tr>
    </tfoot>
</table>
