<!DOCTYPE html>
<html lang="fr">
<head>
<meta charset="utf-8">
<style>
* { margin:0; padding:0; box-sizing:border-box; }
body { font-family: DejaVu Sans, sans-serif; font-size:10px; color:#1e293b; padding:24px; }
table { width:100%; border-collapse:collapse; font-size:9px; }
thead th { background:#f1f5f9; padding:6px 8px; text-align:left; font-size:8px; text-transform:uppercase; color:#64748b; font-weight:700; border-bottom:1px solid #cbd5e1; }
thead th.right { text-align:right; }
tbody tr:nth-child(even) { background:#f8fafc; }
tbody td { padding:5px 8px; border-bottom:1px solid #f1f5f9; vertical-align:middle; }
tbody td.right { text-align:right; }
tfoot td { padding:6px 8px; font-weight:700; border-top:2px solid #cbd5e1; background:#f1f5f9; }
tfoot td.right { text-align:right; }
.footer { margin-top:20px; border-top:1px solid #e2e8f0; padding-top:8px; font-size:8px; color:#9ca3af; text-align:center; }
.badge { display:inline-block; padding:2px 5px; border-radius:3px; font-size:7px; font-weight:700; }
.badge-green { background:#dcfce7; color:#15803d; }
.badge-orange { background:#ffedd5; color:#c2410c; }
.badge-yellow { background:#fef9c3; color:#a16207; }
.badge-red { background:#fee2e2; color:#b91c1c; }
</style>
</head>
<body>

@php $reportTitle = 'LISTE DES FACTURES'; @endphp
@include('reports._pdf_header')

<table>
    <thead>
        <tr>
            <th>N° Facture</th>
            <th>Client</th>
            <th>N° Réparation</th>
            <th>Statut</th>
            <th class="right">Total (F)</th>
            <th class="right">Payé (F)</th>
            <th class="right">Reste (F)</th>
            <th>Date</th>
        </tr>
    </thead>
    <tbody>
        @forelse($invoices as $inv)
        <tr>
            <td style="font-weight:700;">{{ $inv->numero_facture }}</td>
            <td>{{ optional($inv->client)->nom ?? optional($inv->repair)->client_nom ?? '—' }}</td>
            <td>{{ optional($inv->repair)->numeroReparation ?? '—' }}</td>
            <td>
                @if($inv->statut === 'soldee')
                <span class="badge badge-green">Soldée</span>
                @elseif($inv->statut === 'partielle')
                <span class="badge badge-orange">Partielle</span>
                @elseif($inv->statut === 'annulee')
                <span class="badge badge-red">Annulée</span>
                @else
                <span class="badge badge-yellow">En attente</span>
                @endif
            </td>
            <td class="right">{{ number_format($inv->montant_final ?? $inv->montant_total ?? 0, 0, ',', ' ') }}</td>
            <td class="right" style="color:#16a34a;">{{ number_format($inv->montant_paye, 0, ',', ' ') }}</td>
            <td class="right" style="color:{{ $inv->reste_a_payer > 0 ? '#dc2626' : '#94a3b8' }};">{{ number_format($inv->reste_a_payer, 0, ',', ' ') }}</td>
            <td>{{ $inv->created_at->format('d/m/Y') }}</td>
        </tr>
        @empty
        <tr><td colspan="8" style="text-align:center; color:#94a3b8; padding:12px;">Aucune facture.</td></tr>
        @endforelse
    </tbody>
    <tfoot>
        <tr>
            <td colspan="4">TOTAL ({{ $invoices->count() }} facture(s))</td>
            <td class="right">{{ number_format($invoices->sum(fn($i) => $i->montant_final ?? $i->montant_total ?? 0), 0, ',', ' ') }}</td>
            <td class="right">{{ number_format($invoices->sum('montant_paye'), 0, ',', ' ') }}</td>
            <td class="right">{{ number_format($invoices->sum('reste_a_payer'), 0, ',', ' ') }}</td>
            <td></td>
        </tr>
    </tfoot>
</table>

<div class="footer">Généré le {{ now()->format('d/m/Y à H:i') }} — {{ $companyInfo['nom'] ?? 'MOMO TECH SERVICE' }}</div>
</body>
</html>
