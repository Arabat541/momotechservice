...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-900">Modifier — {{ $client->nom }}</h1>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <form method="POST" action="{{ route('clients.update', $client->id) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="telephone" value="{{ old('telephone', $client->telephone) }}" required
                        class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 @error('telephone') border-red-500 @enderror">
                ...
                <div>
                    ...
                    <select name="type" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                        ...
                        <option value="revendeur" {{ old('type', $client->type) === 'revendeur' ? 'selected' : '' }}>Revendeur</option>
                    </select>
                </div>
            </div>

            <div x-data="{ type: '{{ old('type', $client->type) }}' }">
                <div x-show="type === 'revendeur'" class="grid grid-cols-2 gap-4 pt-2">
                    <div>
                        ...
                        <input type="text" name="nom_boutique" value="{{ old('nom_boutique', $client->nom_boutique) }}"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Limite de crédit (F CFA)</label>
                        <input type="number" name="credit_limite" value="{{ old('credit_limite', $client->credit_limite) }}" min="0" step="1000"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500">
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-3 pt-2">
                ...
                <a href="{{ route('clients.show', $client->id) }}" class="text-gray-600 hover:text-gray-800 px-6 py-2 rounded-lg border border-gray-300">
                    Annuler
                </a>
            </div>
        </form>
